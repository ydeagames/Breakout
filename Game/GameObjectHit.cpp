...
static BOOL GameObject_IsHitCirclePoint(const GameObject* circle, const Vec2* p)
{
	...
	return circle->pos.LengthSquaredTo(*p) < r1*r1;
}
...
static BOOL GameObject_IsHitBoxCircle(const GameObject* box, const GameObject* circle)
{
	...
	else
	{
		...
		GameObject boxcircle = GameObject_Create(box->pos, {}, { diagonal, diagonal });
...
static BOOL GameObject_IsHitCircle(const GameObject* obj1, const GameObject* obj2)
{
	...
	return obj1->pos.LengthSquaredTo(obj2->pos) < (r1 + r2)*(r1 + r2);
}

// <線オブジェクト×円オブジェクト当たり判定>
static BOOL GameObject_IsHitLineCircle(const GameObject* line, const GameObject* circle)
{
	Vec2 pointA = { GameObject_GetRawX(line, LEFT), GameObject_GetRawY(line, TOP) };
	Vec2 pointB = { GameObject_GetRawX(line, RIGHT), GameObject_GetRawY(line, BOTTOM) };

	//ベクトルを生成
	Vec2 vecAB = pointB - pointA;
	Vec2 vecAP = circle->pos - pointA;
	Vec2 vecBP = circle->pos - pointB;
	...
	float lenAX = normalAB.Dot(vecAP);
	...
	if (lenAX < 0) {
		...
		shortestDistance = GetAbsF(normalAB.Cross(vecAP));
	}

	{
		//Xの座標を求める(AXの長さより計算）
		Vec2 pointX = { pointA.x + (normalAB.x * lenAX), pointA.y + (normalAB.y * lenAX) };
...
static int ccw(const Vec2* p0, const Vec2* p1, const Vec2* p2)
{
	...
	if (cross > EPS)
		...
		sw = 2;
	else if (a.LengthSquared() < b.LengthSquared())
		sw = -2;
	return sw;
}
...
static BOOL GameObject_IsHitLineBox(const GameObject* line, const GameObject* box)
{
	Vec2 p1 = { GameObject_GetX(box, LEFT), GameObject_GetY(box, TOP) };
	Vec2 p2 = { GameObject_GetX(box, LEFT), GameObject_GetY(box, BOTTOM) };
	Vec2 p3 = { GameObject_GetX(box, RIGHT), GameObject_GetY(box, BOTTOM) };
	Vec2 p4 = { GameObject_GetX(box, RIGHT), GameObject_GetY(box, TOP) };
	Vec2 p5 = { GameObject_GetRawX(line, LEFT), GameObject_GetRawY(line, TOP) };
	Vec2 p6 = { GameObject_GetRawX(line, RIGHT), GameObject_GetRawY(line, BOTTOM) };
...
static BOOL GameObject_IsHitLine(const GameObject* line1, const GameObject* line2)
{
	...
	Vec2 seg1_s = { GameObject_GetRawX(line1, LEFT), GameObject_GetRawY(line1, TOP) };
	Vec2 seg1_s1 = { GameObject_GetRawX(line1, RIGHT), GameObject_GetRawY(line1, BOTTOM) };
	Vec2 seg2_s = { GameObject_GetRawX(line2, LEFT), GameObject_GetRawY(line2, TOP) };
	Vec2 seg2_s1 = { GameObject_GetRawX(line2, RIGHT), GameObject_GetRawY(line2, BOTTOM) };

	Vec2 v = seg2_s - seg1_s;
	Vec2 seg1_v = seg1_s1 - seg1_s;
	Vec2 seg2_v = seg2_s1 - seg2_s;

	float Crs_v1_v2 = seg1_v.Cross(seg2_v);

	if (Crs_v1_v2 != 0.0f) {
		float Crs_v_v1 = v.Cross(seg1_v);
		float Crs_v_v2 = v.Cross(seg2_v);
